...
* если `Exit` &mdash; установить в переданный `Ezh`-ом `Environment` состояния выхода `EXITING`, после чего перейти к последнему пункту;
